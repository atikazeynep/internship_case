...
const MapComponent = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://localhost:8080/earthquake/extraordinary');
        if (!response.ok) {
          throw new Error('Failed to fetch earthquake data');
        }
        const data = await response.json();
        setEarthquakeData(data);
        setNoEarthquakes(data.length === 0);
      } catch (error) {
  ...
  const groupEarthquakes = (data) => {
    ...
    if (Array.isArray(data)) {
      data.forEach((earthquake) => {
        const isAbnormal = earthquake.intensity > 5.0;
  ...
  return (
    <MapContainer center={[0, 0]} zoom={2} style={{ height: '100vh' }}>
      ...
      {noEarthquakes ? (
        <Popup position={[0, 0]}>
          <div>There is no extraordinary/abnormal earthquake right now</div>
        </Popup>
      ) : (
